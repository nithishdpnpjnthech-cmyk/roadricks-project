...
function renderHeroSlider() {
  ...
  sliderImages = data.heroImages.filter(img => img && img.trim() !== '');

  if (sliderImages.length === 0) {
    slider.innerHTML =
      '<div class="hero-slide active" style="background:linear-gradient(135deg,#161616,#252525)"></div>';
    data.currentSlide = 0;
    return;
  }

  if (data.currentSlide >= sliderImages.length) {
    data.currentSlide = 0;
  }
  ...
  sliderImages.forEach((img, i) => {
    ...
    if (i === data.currentSlide) slide.classList.add('active');
    slide.style.backgroundImage = `url("${img}")`;
    slider.appendChild(slide);

    const dot = document.createElement('button');
    dot.className = 'hero-dot';
    if (i === data.currentSlide) dot.classList.add('active');
